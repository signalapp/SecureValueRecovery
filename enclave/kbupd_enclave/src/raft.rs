      ...
      NodeId: Ord + Clone + fmt::Display,
{
        ...
        match self.leadership {
            Candidate { .. } => {                                               // /\ state[i] = Candidate
                let vote_request_msg = RaftMessage {                            // /\ Send([
                    ...
                    inner: Some(raft_message::Inner::VoteRequest(VoteRequest {  //          mtype         |-> RequestVoteRequest,
    ...
    pub fn append_entries(&mut self,
        ...
        if let Leader(leader_state) = &mut self.leadership {                    // /\ state[i] = Leader
            ...
            let append_request_msg = RaftMessage {                              //    IN Send([
                group: self.group.clone(),
                term:  self.current_term,                                       //             mterm          |-> currentTerm[i],
                inner: Some(raft_message::Inner::AppendRequest(AppendRequest {  //             mtype          |-> AppendEntriesRequest,
    ...
    fn handle_vote_request(&mut self,
        ...
        let message = RaftMessage {                                             // /\ Reply([
            group: self.group.clone(),
            term:  self.current_term,                                           //           mterm        |-> currentTerm[i],
            inner: Some(raft_message::Inner::VoteResponse(VoteResponse {        //           mtype        |-> RequestVoteResponse,
    ...
    fn handle_append_request(&mut self,
             ...
             !log_ok))                                                          //                /\ \lnot logOk
        {
            ...
            let message = RaftMessage {                                         //                /\ Reply([
                group: self.group.clone(),
                term:  self.current_term,                                       //                          mterm           |-> currentTerm[i],
                inner: Some(raft_message::Inner::AppendResponse(AppendResponse {//                          mtype           |-> AppendEntriesResponse,
    ...
    pub fn receive(&mut self,
        ...
        let reply = match msg.inner {
            Some(raft_message::Inner::VoteRequest(request)) =>                  //    \/ /\ m.mtype = RequestVoteRequest
                self.handle_vote_request(msg.term, request, from),              //       /\ HandleRequestVoteRequest(i, j, m)
            Some(raft_message::Inner::VoteResponse(response)) => {              //    \/ /\ m.mtype = RequestVoteResponse
            ...
            Some(raft_message::Inner::AppendRequest(request)) =>                //    \/ /\ m.mtype = AppendEntriesRequest
                self.handle_append_request(msg.term, request, from),            //       /\ HandleAppendEntriesRequest(i, j, m)
            Some(raft_message::Inner::AppendResponse(response)) => {            //    \/ /\ m.mtype = AppendEntriesResponse
                match self.drop_stale_response(msg.term, response) {            //       /\ \/ DropStaleResponse(i, j, m)
                    Ok(())        => None,
                    Err(response) =>
