        ...
        "info" | "status" => {
            ...
            let connection = connect_future
                .and_then(move |framed: ControlFramed| {
                    daemon_call(framed, control_request)
                        .map_err(|error: io::Error| {
            ...
            let connection = connect_future
                ...
                .map(move |(reply, _framed): (ControlReply, ControlFramed)| {
                    let result = match reply.data {
                        Some(control_reply::Data::XferControlReply(reply)) => {
                            match UntrustedXferReplyStatus::from_i32(reply.status) {
                            ...
                            let future: Box<dyn Future<Item = (), Error = ()> + Send + 'static> = match request.subcommand.as_str() {
                                "create" => {
                                    let create_backup_request = transaction_control_request::Data::CreateBackupRequest(CreateBackupRequest {
                                        backup_id: BackupId { id: unwrap_or_random_bytes(32, request.backup_id.clone()) },
                                    });
                                    let future = send_transaction_request(request_tx.clone(), request.enclave_name.clone(), request_id, create_backup_request)
                                        .map(|_reply| ());
                                    Box::new(future)
                                }
                                "delete" => {
                                    ...
                                    let future = send_transaction_request(request_tx.clone(), request.enclave_name.clone(), request_id, delete_backup_request)
                                        .map(|_reply| ());
                                    Box::new(future)
                                }
                                _ => {
...
fn print_info(maybe_enclave_name: Option<String>, status: GetStatusControlReply) {
    let maybe_enclave_status = if let Some(enclave_name) = &maybe_enclave_name {
        status.enclaves.into_iter().find(|enclave_status: &EnclaveStatus| &enclave_status.name == enclave_name)
    } else {
...
fn client_request(request_tx: ControlClientSender,
    ...
    create_backup_future
        ...
        .and_then(move |request: kbupd_client::Request| {
            ...
            negotiate_client_request(request_tx, enclave_name, request_id, client_pubkey)
                .map(move |negotiation: kbupd_client::RequestNegotiation| {
                    (request, negotiation)
                })
        })
        .and_then(move |(request, negotiation): (kbupd_client::Request, kbupd_client::RequestNegotiation)| {
            ...
            send_client_request(request_tx_1, enclave_name_1, request_id, backup_id, client, random, negotiation, request)
        })
...
fn send_transaction_request(request_tx:    ControlClientSender,
                            enclave_name:  String,
                            request_id:    u64,
                            ...
                            -> impl Future<Item = TransactionControlReply, Error = ()>
{
    ...
    (request_tx.call(control_request))
        .and_then(move |reply: ControlReply| {
...
fn negotiate_client_request(request_tx:    ControlClientSender,
                            enclave_name:  String,
                            request_id:    u64,
                            ...
                            -> impl Future<Item = kbupd_client::RequestNegotiation, Error = ()>
{
    ...
    (request_tx.call(control_request))
        .and_then(move |reply: ControlReply| {
            parse_negotiate_client_reply(reply)
...
fn send_client_request(request_tx:   ControlClientSender,
                       enclave_name: String,
                       request_id:   u64,
                       backup_id:    Vec<u8>,
                       ...
                       -> impl Future<Item = kbupd_client::Response, Error = ()>
{
    ...
    client.request(&mut random, negotiation, request)
          .into_future()
          .map_err(move |error| {
              error!("error building client request {}: {}", request_id, error);
          })
          .and_then(move |(encrypted_request, pending_request): (kbupd_client::EncryptedRequest, kbupd_client::PendingRequest)| {
              let control_request = ControlRequest {
                  id:   request_id,
              ...
              request_tx.call(control_request)
                        .map(|reply: ControlReply| (reply, pending_request))
          })
          ...
          .and_then(move |(encrypted_message, pending_request): (kbupd_client::EncryptedMessage, kbupd_client::PendingRequest)| {
              pending_request
...
fn parse_client_reply(reply: ControlReply) -> Result<kbupd_client::EncryptedMessage, failure::Error> {
    match reply.data {
        Some(control_reply::Data::ClientEncryptedReply(ClientEncryptedReply { encrypted_message })) => {
            Ok(kbupd_client::EncryptedMessage {
        ...
        _ => {
            Err(failure::format_err!("bad client reply: {:?}", reply.data))
        }
    }
}
...
fn control_client(framed: ControlFramed,
    ...
    framed_rx
        ...
        .select(request_rx.map(|(request, reply_tx): (ControlRequest, oneshot::Sender<ControlReply>)| ControlMessage::Request(request, reply_tx)).map_err(|()| unreachable!()))
        .fold(state, |mut state: ControlClientState, message: ControlMessage| {
            match message {
                ControlMessage::Request(request, reply_tx) => {
                    ...
                    let future = framed_tx
                        .send(request)
                        .map(move |framed_tx: SplitSink<ControlFramed>| {
                            ControlClientState { framed_tx, requests }
                        });
...
fn daemon_call(framed:  ControlFramed, request: ControlRequest)
               -> impl Future<Item = (ControlReply, ControlFramed), Error = io::Error>
{
    framed.send(request)
          .and_then(|framed: ControlFramed| {
              framed.into_future()
                    .map_err(|(error, _framed): (io::Error, ControlFramed)| error)
                    .and_then(|(reply, framed): (Option<ControlReply>, ControlFramed)| {
                        reply.ok_or(io::ErrorKind::UnexpectedEof.into())
...
fn parse_arguments() -> clap::ArgMatches<'static> {
    let enclave_name_argument =
        clap::Arg::with_name("enclave_name")
        .takes_value(true)
        .long("enclave-name")
    ...
    let detailed_memory_status_argument =
        clap::Arg::with_name("detailed_memory_status")
    ...
    let metrics_subcommand =
        clap::SubCommand::with_name("metrics")
        .about("dump a metrics snapshot in JSON");

    let xfer_start_subcommand =
    ...
    let xfer_pause_subcommand =
        clap::SubCommand::with_name("pause")
        .about("pause an ongoing outgoing transfer");

    let xfer_resume_subcommand =
        clap::SubCommand::with_name("resume")
        .about("resume a paused outgoing transfer");

    let xfer_finish_subcommand =
        clap::SubCommand::with_name("finish")
        .about("finish a transfer, tearing down its state, allowing acceptance of a new outgoing transfer");

    let xfer_subcommand =
        clap::SubCommand::with_name("xfer")
    ...
    let peer_node_id_argument =
        clap::Arg::with_name("peer_node_id")
        .required(true)
        .takes_value(true)
        .long("peer-node-id")
    ...
    let peer_address_argument =
        clap::Arg::with_name("peer_address")
        .takes_value(true)
        .long("peer-address")
        .value_name("address")
        .help("ip[:port] address of peer to reconnect to");

    let reconnect_peer_subcommand =
        clap::SubCommand::with_name("reconnect-peer")
        .arg(enclave_name_argument.clone())
        .arg(peer_node_id_argument.clone())
        .arg(peer_address_argument)
        .about("force reconnection to a specified peer");

    let disconnect_peer_subcommand =
        clap::SubCommand::with_name("disconnect-peer")
        .arg(enclave_name_argument.clone())
        .arg(peer_node_id_argument)
        .about("force disconnection from a specified peer");

    let backup_id_argument =
        clap::Arg::with_name("backup_id")
        .takes_value(true)
        .long("backup-id")
    ...
    let request_count_argument =
        clap::Arg::with_name("request_count")
        .takes_value(true)
        .default_value("1")
        .long("request-count")
    ...
    let backup_data_argument =
        clap::Arg::with_name("backup_data")
        .takes_value(true)
        .long("backup-data")
    ...
    let backup_pin_argument =
        clap::Arg::with_name("backup_pin")
        .takes_value(true)
        .long("backup-pin")
    ...
    let backup_tries_argument =
        clap::Arg::with_name("backup_tries")
        .takes_value(true)
        .default_value("10")
        .long("backup-tries")
