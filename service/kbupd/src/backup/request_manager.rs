...
impl BackupRequestManager {
                         ...
                         reply_tx:   oneshot::Sender<Result<Option<KeyBackupResponse>, KeyBackupError>>)
    {
        match self.requests.entry(backup_id) {
            hash_map::Entry::Occupied(mut request_state_entry) => {
                match request_state_entry.get_mut() {
                    RequestState::Pending(pending_request_state)
                        if pending_request_state.request_id == request_id =>
                    {
                        pending_request_state.add_waiter(reply_tx);
                    }
                    RequestState::Finished(finished_request_state)
                        if finished_request_state.request_id == request_id =>
                    {
    ...
    pub fn finish_request(&mut self,
                          backup_id:  BackupId,
                          request_id: Vec<u8>,
                          result:     Result<KeyBackupResponse, KeyBackupError>)
    {
        ...
        let pending_request_state = match request_state {
            RequestState::Pending(pending_request_state)
                if pending_request_state.request_id == request_id => pending_request_state,
            _ => return,
        };
...
mod test {
    ...
    fn sleep(runtime: &mut Runtime, duration: Duration) {
        runtime.block_on(Delay::new(Instant::now() + Duration::from_millis(1) + duration)).unwrap()
    }
    ...
    fn test_expire_request() {
        ...
        request_manager.sender().cast(move |request_manager: &mut BackupRequestManager| {
