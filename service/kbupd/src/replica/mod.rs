...
impl ReplicaService {
    pub fn start(config:               ReplicaConfig,
        ...
        let mut runtime = tokio::runtime::current_thread::Builder::new().build().context("error starting tokio runtime")?;
        ...
        let maybe_intel_client = if !config.attestation.disabled {
            let intel_client_proxy = TlsClientProxySpawner::new(cmdline_config.kbuptlsd_bin_path.to_owned(), TlsClientProxyArguments::Config {
        ...
        let peer_tls_client = TlsClient::new(cmdline_config.kbuptlsd_bin_path.to_owned(), peer_tls_client_args).context("error creating peer tls client")?;
        ...
        let enclave_replica_config = EnclaveReplicaConfig {
            ...
            attestation_expiry_ticks:  util::duration::as_ticks(Duration::from_millis(config.enclave.attestationExpiryCommitIntervalMs), timer_tick_interval),
            request_quote_ticks:       util::duration::as_ticks(Duration::from_millis(config.enclave.attestationExpiryCommitIntervalMs), timer_tick_interval),
        ...
        info!("starting enclave with mrenclave {} with timer tick interval {}ms and {:#?}",
              &enclave_mrenclave, timer_tick_interval.as_millis(), &enclave_replica_config);
        ...
        let enclave_thread           = thread::spawn(move || -> Result<(), failure::Error> {
            let mut enclave = Enclave::new(ENCLAVE_NAME.to_string(), &enclave_path.to_string_lossy(), enclave_debug, enclave_spid, enclave_peer_manager_tx, attestation_manager_tx).context("error creating enclave")?;
            let node_id     = enclave.start_replica(start_replica_request).context("error starting replica in enclave")?;
        ...
        let peer_listener       = PeerListener::new(peer_listen_address, cmdline_config.kbuptlsd_bin_path.to_owned(), tls_max_connections, peer_tls_server_args, peer_manager_tx.clone()).context("error starting peer listener")?;
        let peer_manager        = PeerManager::new(peer_manager_tx.clone(), ENCLAVE_NAME.to_string(), enclave_manager_tx.clone(), node_id, peer_tls_client.clone());
        ...
        if let Some(metrics_config) = config.metrics {
            for reporter_config in metrics_config.reporters {
                ...
                let reporter_tls_proxy = TlsClientProxySpawner::new(cmdline_config.kbuptlsd_bin_path.to_owned(), TlsClientProxyArguments::NoConfig {
                ...
                let reporter_interval = json_reporter_config.intervalSeconds.map(Duration::from_secs)
                                                                            .unwrap_or(DEFAULT_METRICS_INTERVAL);
                let json_reporter     = JsonReporter::new(&json_reporter_config.token, &json_reporter_config.hostname, cmdline_config.full_hostname, reporter_tls_connector).context("error creating metrics json reporter")?;
        ...
        let unix_signals = unix_signal::handle_signals(vec![
            SIGHUP,
            SIGINT,
            SIGQUIT,
            SIGTERM,
        ...
        let handled_unix_signals = unix_signals.for_each(move |signum: signal::Signal| {
            match signum {
                ...
                SIGQUIT |
                SIGTERM
                    if exit_signals_enabled =>
                {
                    warn!("shutting down due to unix signal {}", signum);
                    let _ignore = enclave_manager_tx_2.cast(|enclave_manager: &mut EnclaveManager| {
                        enclave_manager.stop()
                    });
                }
                _ => {
        ...
        let source_partition = if let Some(source_partition_config) = config.enclave.sourcePartition {
            let source_partition_range = PartitionKeyRangePb {
                first: BackupId { id: source_partition_config.firstBackupId.to_vec() },
                last:  BackupId { id: source_partition_config.lastBackupId.to_vec() },
            };

            let source_replica_addresses =
                source_partition_config.replicas.into_iter()
                                                .map(|peer| peer.hostPort)
                                                .collect();
            ...
            let source_partition = source_node_ids.map(move |source_node_ids: Vec<NodeId>| {
                ...
                let _ignore = peer_manager_tx.cast(move |peer_manager: &mut PeerManager| {
                    peer_manager.set_source_partition(source_node_ids_2)
                });
        ...
        let peer_replica_addresses =
            config.enclave.replicas.into_iter()
                                   .map(|peer| peer.hostPort)
                                   .collect();
        ...
        let replica_group_started = discovered_peers.map(move |(source_partition, peer_node_ids): (Option<SourcePartitionConfig>, Vec<NodeId>)| {
        ...
        Ok(Self { runtime, enclave_thread_joiner })
    }
