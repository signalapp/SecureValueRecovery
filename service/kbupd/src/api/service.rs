...
where BackupManagerTy: BackupManager<User = SignalUser> + Clone + Send + 'static,
{
    pub fn new(signal_user_authenticator: Arc<SignalUserAuthenticator>, backup_manager: BackupManagerTy, deny_backup: bool, rate_limiters: SignalApiRateLimiters) -> Self {
        ...
        router.add("/v1/ping", Self::api_handler(move |_service, _params, request| {
            match *request.method() {
                Method::GET => Some(Self::get_request_handler(&AnonymousUserAuthenticator, |service, _params, user, request| {
                    service.ping(user, request)
                })),
                _ => None,
            }
        }));
        router.add("/v1/token/:enclave_name", Self::api_handler(move |service, _params, request| {
            match *request.method() {
                Method::GET => Some(Self::get_request_handler(service.signal_user_authenticator.clone(), |service, params, user, request| {
                    service.get_token(&params["enclave_name"], user, request)
                })),
                _ => None,
            }
        }));
        router.add("/v1/attestation/:enclave_name", Self::api_handler(move |service, _params, request| {
            match *request.method() {
                Method::PUT => Some(Self::request_handler(service.signal_user_authenticator.clone(), |service, params, _parts, user, request| {
                    service.get_attestation(&params["enclave_name"], user, request)
                })),
                _ => None,
            }
        }));
        router.add("/v1/backup/:enclave_name", Self::api_handler(move |service, _params, request| {
            match *request.method() {
                Method::PUT => Some(Self::request_handler(service.signal_user_authenticator.clone(), |service, params, _parts, user, request| {
                    service.put_backup_request(&params["enclave_name"], user, request)
                })),
                _ => None,
            }
        }));
    ...
    fn get_attestation(&self, enclave_name: &str, user: SignalUser, request: RemoteAttestationRequest)
                       -> impl Future<Item = Result<RemoteAttestationResponse, Response<Body>>, Error = failure::Error> {
        let timer    = GET_ATTESTATION_TIMER.time();
        let username = user.username.clone();
        let limit    = self.rate_limiters.attestation.sync_call(move |rate_limiter: &mut RateLimiter| {
            Self::handle_ratelimit_result(rate_limiter.validate(&username, 1))
        });
    ...
    fn put_backup_request(&self, enclave_name: &str, user: SignalUser, request: KeyBackupRequest)
    ...
    where F: Fn(&Self, &route_recognizer::Params, &Request<Body>) -> Option<H> + Send + Clone + 'static,
          H: ApiHandler<ApiService = Self>,
    {
        Box::new(SignalApiHandler::new(move |service: &Self, params: route_recognizer::Params, request: Request<Body>| {
            match handler(service, &params, &request) {
    ...
    fn get_request_handler<AuthTy, ResTy, F, FRes>(authenticator: impl Deref<Target = AuthTy> + Clone + Send + Sync + 'static,
                                                   handler:       F)
                                                   -> impl ApiHandler<ApiService = Self>
    where ResTy:  Serialize + 'static,
          AuthTy: Authenticator,
          F:      Fn(&Self, &route_recognizer::Params, AuthTy::User, Request<Body>)
                     -> FRes + Send + Clone + 'static,
    ...
    fn request_handler<AuthTy, ReqTy, ResTy, F, FRes>(authenticator: impl Deref<Target = AuthTy> + Clone + Send + Sync + 'static,
                                                      handler:       F)
                                                      -> impl ApiHandler<ApiService = Self>
    where ReqTy:  for<'de> Deserialize<'de> + Send + 'static,
          ResTy:  Serialize + 'static,
          AuthTy: Authenticator,
          F:      Fn(&Self, &route_recognizer::Params, request::Parts, AuthTy::User, ReqTy)
                     -> FRes + Send + Clone + 'static,
          FRes:   Future<Item = Result<ResTy, Response<Body>>, Error = failure::Error> + Send + 'static,
    {
        SignalApiHandler::new(move |service: &Self, params: route_recognizer::Params, request: Request<Body>| {
    ...
    fn handle_ratelimit_result(result: Result<(), RateLimitError>) -> Result<Option<Response<Body>>, failure::Error> {
        match result {
            ...
            Err(error @ RateLimitError::InternalError) => {
                Err(error.into())
            }
        }
    }

    fn read_request<ReqTy>(request: Request<Body>)
    ...
    where ReqTy: for<'de> Deserialize<'de>,
    {
        ...
        let response     = request_data.and_then(|data: Chunk| {
            match serde_json::from_slice(&data) {
                Ok(deserialized) => {
                    future::Either::A(Ok(Ok((request_parts, deserialized))).into_future())
                }
    ...
    fn handle_result<ResTy>(handler_result: Result<Result<ResTy, Response<Body>>, failure::Error>)
...
trait ApiHandler: Send {
    type ApiService;
    fn handle(&self, service: &Self::ApiService, params: route_recognizer::Params, request: Request<Body>)
...
where F:    Fn(&ApiServiceTy, route_recognizer::Params, Request<Body>) -> FRes + Send + Clone + 'static,
      FRes: Future<Item = Response<Body>, Error = failure::Error> + Send + 'static,
      ApiServiceTy: Send + 'static,
{
    type ApiService = ApiServiceTy;
    fn handle(&self, service: &ApiServiceTy, params: route_recognizer::Params, request: Request<Body>)
              -> Box<dyn Future<Item = Response<Body>, Error = failure::Error> + Send> {
...
mod test {
    ...
    impl BackupManager for actor::Sender<BackupManagerMock<SignalUser>> {
        ...
        fn get_token(&self, enclave_name: String, user: &Self::User) -> Box<dyn Future<Item = GetTokenResponse, Error = EnclaveTransactionError> + Send> {
            let user        = user.clone();
            let call_result = self.sync_call(move |backup_manager: &mut BackupManagerMock<SignalUser>| Ok(backup_manager.get_token(enclave_name, &user)));
        ...
        fn get_attestation(&self, enclave_name: String, user: &Self::User, request: RemoteAttestationRequest) -> Box<dyn Future<Item = RemoteAttestationResponse, Error = RemoteAttestationError> + Send> {
            let user        = user.clone();
            let call_result = self.sync_call(move |backup_manager: &mut BackupManagerMock<SignalUser>| Ok(backup_manager.get_attestation(enclave_name, &user, request)));
            Box::new(call_result.then(|result: Result<_, futures::Canceled>| result.unwrap()))
        }
        fn put_backup_request(&self, enclave_name: String, user: &Self::User, request: KeyBackupRequest) -> Box<dyn Future<Item = KeyBackupResponse, Error = KeyBackupError> + Send> {
            let user        = user.clone();
            let call_result = self.sync_call(move |backup_manager: &mut BackupManagerMock<SignalUser>| Ok(backup_manager.put_backup_request(enclave_name, &user, request)));
            Box::new(call_result.then(|result: Result<_, futures::Canceled>| result.unwrap()))
        }
    }
    ...
    impl SignalApiServiceTestBuilder {
        ...
        pub fn build(self) -> SignalApiServiceTest {
            ...
            let rate_limiters = SignalApiRateLimiters {
                token:       actor::spawn(RateLimiter::new("token",       LeakyBucketParameters { size: self.ratelimiter_size, leak_rate: self.ratelimiter_size as f64 }), &runtime_handle).unwrap(),
                attestation: actor::spawn(RateLimiter::new("attestation", LeakyBucketParameters { size: self.ratelimiter_size, leak_rate: self.ratelimiter_size as f64 }), &runtime_handle).unwrap(),
                backup:      actor::spawn(RateLimiter::new("backup",      LeakyBucketParameters { size: self.ratelimiter_size, leak_rate: self.ratelimiter_size as f64 }), &runtime_handle).unwrap(),
            };
    ...
    impl SignalApiServiceTest {
        ...
        pub fn serve(&self, incoming: mocks::AsyncPipeIncoming) -> impl Future<Item = (), Error = ()> {
            ...
            server.map_err(|error: hyper::Error| {
                panic!("hyper server error: {}", error)
            })
        }
    ...
    fn test_not_found() {
        ...
        let request  = Request::get("http://invalid/nonexistant")
            .header(header::AUTHORIZATION, mocks::basic_auth(&test.valid_user.username, &test.valid_user))
            .body(Body::empty())
            .unwrap();
    ...
    fn test_get_token_unauthorized() {
        let mut test = SignalApiServiceTest::builder().build();
        let request  = Request::get("http://invalid/v1/token/test_enclave")
            .header(header::AUTHORIZATION, mocks::basic_auth(&test.valid_user.username, "invalid_password"))
            .body(Body::empty())
            .unwrap();
    ...
    fn test_get_token_ratelimit_exceeded() {
        ...
        test.scenario.expect(test.backup_manager.get_token("test_enclave".to_string(), ANY)
    ...
    fn test_get_token_valid() {
        ...
        let client        = test.client();
        let response_data = test.runtime.block_on(client.request(request).and_then(|response: Response<Body>| {
    ...
    fn test_get_attestation_ratelimit_exceeded() {
        ...
        test.scenario.expect(test.backup_manager.get_attestation("test_enclave".to_string(), ANY, ANY)
                             .and_return(Box::new(future::lazy(|| -> Result<_,_> { panic!("response future was polled") }))));
    ...
    fn test_put_backup_request_bad_method() {
        ...
        let request  = Request::get("http://invalid/v1/backup/test_enclave")
            .header(header::AUTHORIZATION, mocks::basic_auth(&test.valid_user.username, &test.valid_user))
            .body(Body::from(serde_json::to_string(&valid_key_backup_request(KeyBackupRequestType::Backup)).unwrap()))
            .unwrap();
    ...
    fn test_put_backup_request_ratelimit_exceeded() {
        ...
        test.scenario.expect(test.backup_manager.put_backup_request("test_enclave".to_string(), ANY, ANY)
                             .and_return(Box::new(future::lazy(|| -> Result<_,_> { panic!("response future was polled") }))));
    ...
    fn test_put_backup_request_deny_backup() {
        ...
        let restore_request = Request::put("http://invalid/v1/backup/test_enclave")
            .header(header::AUTHORIZATION, mocks::basic_auth(&test.valid_user.username, &test.valid_user))
            .body(Body::from(serde_json::to_string(&valid_key_backup_request(KeyBackupRequestType::Restore)).unwrap()))
            .unwrap();
        let delete_request = Request::put("http://invalid/v1/backup/test_enclave")
            .header(header::AUTHORIZATION, mocks::basic_auth(&test.valid_user.username, &test.valid_user))
            .body(Body::from(serde_json::to_string(&valid_key_backup_request(KeyBackupRequestType::Delete)).unwrap()))
            .unwrap();

        let client = test.client();
        test.runtime.block_on(client.request(backup_request).map(|response: Response<Body>| {
            assert!(response.status().is_server_error());
        })).unwrap();

        let response_data = test.runtime.block_on(client.request(restore_request).and_then(|response: Response<Body>| {
            assert!(response.status().is_success());
            response.into_body().concat2().from_err()
        })).unwrap();
        assert_eq!(mock_backup_response, serde_json::from_slice(&response_data).unwrap());

        let response_data = test.runtime.block_on(client.request(delete_request).and_then(|response: Response<Body>| {
            assert!(response.status().is_success());
            response.into_body().concat2().from_err()
        })).unwrap();
        assert_eq!(mock_backup_response, serde_json::from_slice(&response_data).unwrap());

        let response_data = test.runtime.block_on(client.request(token_request).and_then(|response: Response<Body>| {
            response.into_body().concat2().from_err()
        })).unwrap();
