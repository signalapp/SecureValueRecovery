...
impl HandshakeManager {
    ...
    pub fn fetch_all(self) -> impl Future<Item = Self, Error = failure::Error> + Send + 'static {
        ...
        let quotes_vec     = fetched_sig_rl.and_then(move |()| {
            enclave_tx.call(|enclave_manager: &mut EnclaveManager, reply_tx| {
                enclave_manager.get_next_quotes(reply_tx)
            })
        });

        let quotes = quotes_vec.map(|quotes: Vec<(String, SgxQuote)>| {
            futures::stream::iter_ok(quotes)
        }).flatten_stream();
    ...
    fn fetch(self, enclave_name: String, quote: SgxQuote)
        ...
        let state = signed_quote.then(move |result: Result<SignedQuote, GetQuoteSignatureError>| {
            match result {
                Ok(signed_quote) => {
                    GET_SIGNED_QUOTE_OK_METER.mark();
                    let _ignore = self.enclave_tx.cast(move |enclave_manager: &mut EnclaveManager| {
                        enclave_manager.set_current_quote(enclave_name, signed_quote)
                    });
                    Ok(self)
                }
    ...
    fn fetch_sig_rl(&self) -> impl Future<Item = (), Error = failure::Error> + Send + 'static {
        let gid = self.enclave_tx.call(|enclave_manager: &mut EnclaveManager, reply_tx| {
            enclave_manager.get_sgx_gid(reply_tx)
        });

        let intel_client = self.intel_client.clone();
        let sig_rl       = gid.and_then(move |gid: u32| {
            intel_client.get_signature_revocation_list(gid)
        });

        let enclave_tx = self.enclave_tx.clone();
        let set_sig_rl = sig_rl.map(move |sig_rl: SignatureRevocationList| {
            let _ignore = enclave_tx.cast(|enclave_manager: &mut EnclaveManager| {
                enclave_manager.set_signature_revocation_list(sig_rl)
            });
        });
    ...
    pub fn into_future(self) -> impl Future<Item = (), Error = ()> + Send {
        ...
        let interval_timer = interval_timer_stream.fold(self, |state: Self, _now: Instant| {
            ...
            let refresh_all_result = refresh_all_result.or_else(|(state, error): (Self, RefreshAllError)| {
                match error {
                    RefreshAllError::StaleRevocationList => {
                        info!("fetching new signature revocation list from IAS");
                        let sig_rl       = state.fetch_sig_rl();
                        let fetch_result = sig_rl.then(move |sig_rl_result: Result<(), failure::Error>| {
                            match sig_rl_result {
    ...
    fn refresh_all(self) -> impl Future<Item = Self, Error = (Self, RefreshAllError)> {
        let quotes_vec = self.enclave_tx.call(|enclave_manager: &mut EnclaveManager, reply_tx| {
            enclave_manager.get_next_quotes(reply_tx)
        });

        let quotes = quotes_vec.then(|quotes_result: Result<Vec<(String, SgxQuote)>, failure::Error>| {
            match quotes_result {
        ...
        quotes.fold(self, |state: Self, (enclave_name, sgx_quote): (String, SgxQuote)| {
            state.fetch(enclave_name, sgx_quote).then(|fetch_result: Result<Self, (Self, GetQuoteSignatureError)>| {
                fetch_result.or_else(|(state, error)| {
                    match error {
