...
impl Peer {
    ...
    pub fn send_message(&mut self, data: Vec<u8>, syn: bool, debug_msg: Option<Vec<u8>>) {
        ...
        if syn {
            ...
            debug!("sending message {} to peer {}: {}", id, &self.node_id, String::from_utf8_lossy(debug_msg));
        } else {
        ...
        let message = OutgoingPeerMessage {
            ...
            data: Arc::new(PeerConnectionMessage {
                inner: Some(peer_connection_message::Inner::Data(PeerConnectionData {
                    id,
                    data,
                    syn,
                }))
    ...
    fn inner_poll(&mut self) -> Result<Async<()>, ()> {
        ...
        let state_counter = match &self.state {
            ...
            PeerState::Waiting { .. } |
            PeerState::Connecting { .. } => &*PEERS_CONNECTING_COUNTER,
    ...
    fn step(&mut self) -> Async<Option<PeerState>> {
        match &mut self.state {
            PeerState::Disconnected => {
                self.connect(CONNECT_RETRY_INITIAL_DELAY)
            }
            ...
            PeerState::Connecting { connect, address, max_delay } => {
                let max_delay = *max_delay;
                match connect.poll() {
                    Ok(Async::Ready(stream)) => {
                        match PeerConnection::connect_with_tcp_stream(stream, *address, &self.tls_client) {
                            Ok(connection) => {
                                let new_state = self.connected(connection);
                                Async::Ready(Some(new_state))
                            }
                            Err(connect_error) => {
                                warn!("error connecting to peer at {}: {}", util::OptionDisplay(self.address.as_ref()), connect_error);
            ...
            PeerState::Connected(connected_state) => {
                ...
                if let Some(keepalive) = &mut connected_state.keepalive {
                    match keepalive.poll() {
                        Ok(Async::Ready(())) => {
                            let keepalive = Arc::new(PeerConnectionMessage {
                                inner: None,
                            });
    ...
    fn connect(&self, max_delay: Duration) -> Async<Option<PeerState>> {
        if let Some(address) = &self.address {
            ...
            match util::to_socket_addr(address) {
                Ok(socket_addr) => {
                    Async::Ready(Some(PeerState::Connecting {
    ...
    fn connected(&self, connection: PeerConnection) -> PeerState {
        ...
        match self.pending_outgoing.len() {
            ...
            pending_outgoing_len => {
                info!("resending {} pending outgoing messages to peer {}", pending_outgoing_len, &self.node_id);
            }
        }
        ...
        match message.inner {
            Some(peer_connection_message::Inner::Hello(hello)) => {
                if hello.node_id[..] != self.node_id[..] {
                    warn!("reconnected to node {} at {} but found node {} instead!",
                          &self.node_id, util::OptionDisplay(self.address.as_ref()), &util::ToHex(&hello.node_id));
                    self.address = None;
        ...
        if message_id < self.next_incoming_id {
            if let PeerState::Connected(connected_state) = &mut self.state {
                let ack = Arc::new(PeerConnectionMessage {
                    inner: Some(peer_connection_message::Inner::DataAck(PeerConnectionDataAck {
                        id: message_id,
                    })),
                });
            ...
            warn!("dropping message {} (expected {}) from peer {}", message_id, self.next_incoming_id, &self.node_id);
        }
    }
    ...
    fn send_incoming(&self, message: PeerConnectionData) -> Result<(), ()> {
        ...
        self.enclave_tx.cast(|enclave_manager: &mut EnclaveManager| {
...
impl PeerConnectedState {
    fn new(framed: PeerFramed, queue_capacity: usize) -> Self {
        ...
        Self { framed, queue, keepalive, timeout }
    }
