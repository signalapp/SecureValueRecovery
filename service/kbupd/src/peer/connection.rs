...
impl PeerConnection {
    ...
    pub fn connect(address: &str, tls_client: TlsClient) -> impl Future<Item = Self, Error = io::Error> {
        ...
        let framed      = stream.and_then(move |stream: TcpStream| {
            Self::connect_with_tcp_stream(stream, socket_addr, &tls_client)
        });
        framed.into()
    }
    ...
    pub fn send_hello(self, our_hello: PeerConnectionHello) -> impl Future<Item = Self, Error = io::Error> {
        ...
        let connection = framed.map(move |framed: PeerFramed| {
            Self { framed, sent_hello: true }
        });
        connection
    }
    ...
    pub fn read_hello(self) -> impl Future<Item = (PeerConnectionHello, Self), Error = io::Error> {
        ...
        let connection    = maybe_message.and_then(move |(maybe_message, framed): (Option<PeerConnectionMessage>, PeerFramed)| {
            match maybe_message {
                Some(PeerConnectionMessage { inner: Some(peer_connection_message::Inner::Hello(peer_hello)) }) => {
                    Ok((peer_hello, Self { framed, sent_hello }))
                }
                Some(message) => {
                    Err(io::Error::new(io::ErrorKind::InvalidData, format!("first message on peer connection not a hello: {:?}", message)))
                }
                None => {
                    Err(io::Error::new(io::ErrorKind::Other, "eof before connection hello"))
                }
            }
        });
        connection
...
pub struct ReconnectLooper<F> where F: Send + Clone {
    connect_fn: F,
    max_delay:  Duration,
}

impl<F, Fut, T> ReconnectLooper<F>
where F: FnOnce() -> Fut + Send + Clone,
      Fut: Future<Item = T, Error = ()>,
{
    ...
    fn connect(self) -> impl Future<Item = future::Loop<T, Self>, Error = ()> {
        ...
        let loop_result    = connect_result.then(|result: Result<T, ()>| {
            match result {
                Ok(result) => {
                    future::Either::A(Ok(future::Loop::Break(result)).into_future())
                }
                Err(()) => {
