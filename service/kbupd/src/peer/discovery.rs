...
impl PartitionPeerDiscovery {
    pub fn new(range: Option<PartitionKeyRangePb>, addresses: Vec<String>, tls_client: &TlsClient) -> Self {
        info!("discovering peers for partition with range {} and peers {}", util::OptionDisplay(range.as_ref()), util::ListDisplay(&addresses));
        ...
        for address in addresses.iter().cloned() {
            ...
            let reconnect_looper = ReconnectLooper::new(move || {
                ...
                let peer_hello = connection.and_then(|connection: PeerConnection| {
                ...
                let discovered_peer = peer_hello.map(|(peer_hello, connection): (PeerConnectionHello, PeerConnection)| {
                    DiscoveredPeer { address, peer_hello, connection }
                });
                discovered_peer
            });
    ...
    fn discover_loop(self) -> impl Future<Item = Loop<(PartitionConfig, Self), Self>, Error = ()> + Send + 'static {
        let Self { range, addresses, connecting, mut connected } = self;
    ...
    pub fn finish(self, peer_manager_tx: PeerManagerSender) -> impl Future<Item = (), Error = ()> {
        ...
        connections.for_each(move |discovered_peer: DiscoveredPeer| {
            let _ignore = peer_manager_tx.cast(|peer_manager: &mut PeerManager| {
                peer_manager.add_connection(Some(discovered_peer.address), discovered_peer.peer_hello, discovered_peer.connection);
            });
            Ok(())
        })
    }
