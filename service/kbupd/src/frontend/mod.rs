...
impl FrontendService {
    pub fn start(config:               FrontendConfig,
                 ...
                 -> Result<Self, failure::Error>
    {
        ...
        for enclave_config in config.enclaves {
            ...
            let partitions = runtime.block_on(future::join_all(partition_discoveries))
        ...
        let enclave_thread    = thread::spawn(move || -> Result<(), failure::Error> {
            ...
            for (enclave_config, partitions) in enclave_configs {
                ...
                info!("starting enclave {} with mrenclave {} with timer tick interval {}ms and {:#?}",
                      &enclave_name, &enclave_config.mrenclave, timer_tick_interval.as_millis(),
                ...
                let mut enclave = (Enclave::new(enclave_name.clone(), &enclave_path.to_string_lossy(), enclave_config.debug, enclave_spid, peer_manager_tx.clone(), attestation_manager_tx.clone())
                                   .with_context(|_| format_err!("error creating enclave {}", enclave_name)))?;
                let node_id     = enclave.start_frontend(start_frontend_request, PENDING_REQUESTS_TABLE_ORDER)
                                         .with_context(|_| format_err!("error starting frontend in enclave {}", enclave_name))?;

                let peer_manager        = PeerManager::new(peer_manager_tx, enclave_config.name.to_string(), enclave_manager_channel.sender().clone(), node_id, peer_tls_client.clone());
                let timer_tick_task     = EnclaveTimerTickTask::new(timer_tick_interval, enclave_name, enclave_manager_channel.sender().clone());
        ...
        let handshake_manager = if let Some(handshake_manager) = handshake_manager {
            Some(runtime.block_on(handshake_manager.fetch_all()).context("error fetching quotes from IAS")?)
        } else {
            None
        };
