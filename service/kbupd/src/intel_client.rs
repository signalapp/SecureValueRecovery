...
use futures::prelude::*;
use http::{self, Uri, HeaderMap};
use http::header::{HeaderValue};
use hyper::{Body, Chunk, Client, Method, Response, Request};
...
use crate::*;
use crate::protobufs::kbupd::{IasReport};
use crate::util;

#[derive(Clone)]
pub struct IntelClient {
    host:   Uri,
    client: Client<TlsProxyConnector<HttpConnector>, Body>,
}
...
impl IntelClient {
    pub fn new(host: &str, tls_proxy: TlsClientProxySpawner) -> Result<Self, failure::Error> {
    ...
    pub fn get_signature_revocation_list(&self, gid: u32) -> impl Future<Item = SignatureRevocationList, Error = failure::Error> {
        ...
        let response_data = response.and_then(|response: Response<Body>| {
            response.into_body().concat2()
        }).from_err();

        let decoded_response = response_data.and_then(|data: Chunk| {
            base64::decode(&data).map(SignatureRevocationList).into_future().from_err()
        });
    ...
    fn fetch_quote_signature(&self, quote: &[u8]) -> impl Future<Item = (http::response::Parts, Chunk), Error = failure::Error> {
        ...
        let request = QuoteSignatureRequest {
        ...
        *hyper_request.uri_mut()    = uri;
        hyper_request.headers_mut().insert("Content-Type", HeaderValue::from_static("application/json"));
    ...
    pub fn get_quote_signature(&self, quote: Vec<u8>, accept_group_out_of_date: bool)
...
fn validate_quote_signature(response_parts:           http::response::Parts,
    ...
    if !response_parts.status.is_success() {
        ...
        return Err(QuoteVerificationError::HttpError(response_parts.status, response_parts, response_body_string));
    }
    ...
    let signature = base64::decode(base64_signature.as_bytes())
        .map_err(|_| QuoteVerificationError::InvalidSignature(base64_signature.to_string()))?;

    let certificates = util::pem::decode(&util::percent::decode(pem_certificates.as_bytes()));
    ...
    let parsed_body: QuoteSignatureResponseBody =
        ...
        .map_err(|parse_error| QuoteVerificationError::InvalidJson(parse_error.into()))?;

    if parsed_body.version != 3 {
        return Err(QuoteVerificationError::WrongVersion(parsed_body.version));
    }

    if Some(&parsed_body.isvEnclaveQuoteBody[..]) != quote.get(..SgxQuote::SIZE) {
    ...
    match parsed_body.isvEnclaveQuoteStatus.as_str() {
        ...
        "GROUP_OUT_OF_DATE" | "CONFIGURATION_NEEDED" => {
            if !accept_group_out_of_date {
                return Err(QuoteVerificationError::GroupOutOfDate(parsed_body.isvEnclaveQuoteStatus.clone(), parsed_body));
            }
        }
        "GROUP_REVOKED" => {
            return Err(QuoteVerificationError::GroupOutOfDate(parsed_body.isvEnclaveQuoteStatus.clone(), parsed_body));
