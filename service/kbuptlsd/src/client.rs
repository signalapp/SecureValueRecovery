...
impl TlsClientProxySpawner {
    pub fn new(bin_path: PathBuf, arguments: TlsClientProxyArguments) -> Result<Self, failure::Error> {
        ...
        match &arguments {
            TlsClientProxyArguments::Config { config_file: config_file_path, key_file }  => {
                let config_file   = fs::File::open(config_file_path).with_context(|_| format_err!("error opening config file {}", config_file_path.display()))?;
                let config        = Config::from_reader(config_file).with_context(|_| format_err!("error reading config file {}", config_file_path.display()))?;
                let client_config = config.client.ok_or_else(|| format_err!("config file has no client section: {}", config_file_path.display()))?;
                if client_config.clientCertificatePkcs12.is_none() {
                    let key_file = key_file.as_ref().ok_or_else(|| format_err!("config file has no clientCertificatePkcs12: {}", config_file_path.display()))?;
    ...
    pub fn spawn(&self,
                 target_stream:  impl AsRawFd,
                 target_address: SocketAddr)
                 -> Result<TlsProxyChild, io::Error>
    {
