...
use log::{error, warn, info, debug, log};
...
impl TlsProxyListener {
    pub fn new(listen_host_port: impl ToSocketAddrs,
               bin_path:         PathBuf,
               max_connections:  usize,
               arguments:        TlsProxyListenerArguments)
               -> Result<Self, failure::Error>
    {
        bin_path.metadata().with_context(|_| format!("error opening bin file {}", bin_path.display()))?;

        match &arguments {
            TlsProxyListenerArguments::Config { config_file, key_file }  => {
                config_file.metadata().with_context(|_| format!("error opening config file {}", config_file.display()))?;
                key_file.metadata().with_context(|_| format!("error opening key file {}", key_file.display()))?;
            }
            TlsProxyListenerArguments::NoConfig { ca_file, key_file } => {
                ca_file.metadata().with_context(|_| format!("error opening ca certificate file {}", ca_file.display()))?;
                key_file.metadata().with_context(|_| format!("error opening key file {}", key_file.display()))?;
            }
        }
        ...
        Ok(Self { bin_path, arguments, max_connections, tcp_listener })
    }
    ...
    pub fn proxy_to(self, target_address: SocketAddr) -> impl Future<Item = (), Error = failure::Error> {
        self.into_stream().for_each(move |proxy_child: TlsProxyStream| {
            proxy_child.proxy_to(target_address).or_else(|()| Ok(()))
        })
    }
    pub fn into_stream(self) -> impl Stream<Item = TlsProxyStream, Error = failure::Error> {
        let Self { bin_path, arguments, max_connections, mut tcp_listener } = self;
        ...
        let tcp_connections = tcp_connections.sleep_on_error(LISTEN_RETRY_DELAY).map_err(|()| {
            failure::format_err!("listen error")
        });
        let proxy_children = tcp_connections.filter_map(move |(source_stream, source_address): (std::net::TcpStream, SocketAddr)| {
            ...
            if connection_count >= max_connections {
                warn!("{} => connection dropped due to max connections {}", source_address, connection_count);
                return None;
            }
    ...
    fn log_proxy_stderr(stderr_stream:  TlsProxyStderrStream,
