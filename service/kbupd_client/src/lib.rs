...
impl Client {
    ...
    {
        let (client_key, server_key) = key_agreement(&self.client_privkey,
                                                     &self.client_pubkey,
                                                     &negotiation.server_ephemeral_pubkey,
                                                     &negotiation.server_static_pubkey);

        let ring_server_key = ring::aead::OpeningKey::new(&ring::aead::AES_256_GCM, &server_key)
            .context("invalid server_key")?;
        let ring_client_key = ring::aead::SealingKey::new(&ring::aead::AES_256_GCM, &client_key)
        ...
        ring::aead::open_in_place(&ring_server_key,
                                  nonce,
                                  ring::aead::Aad::empty(), 0,
        ...
        ring::aead::seal_in_place(&ring_client_key,
                                  nonce,
...
impl PendingRequest {
    pub fn decrypt_reply(self, mut reply: EncryptedMessage) -> Result<Response, failure::Error> {
        ...
        let ring_server_key = ring::aead::OpeningKey::new(&ring::aead::AES_256_GCM, &self.server_key)
            .context("invalid server_key")?;
...
fn key_agreement(client_privkey:          &x25519_dalek::StaticSecret,
                 client_pubkey:           &x25519_dalek::PublicKey,
