...
fn main() -> Result<(), failure::Error> {
    ...
    let connect_uri  = arguments.value_of("connect_uri").unwrap_or_default().parse::<Uri>().context("invalid --connect uri")?;
    ...
    let token_secret = parse_argument(arguments.value_of("token_secret"), parse_hex).context("invalid --token-secret")?.unwrap_or_default();
    ...
    match subcommand_name {
        "delete" => {
            ...
            let service_id   = subcommand_arguments.value_of("service_id").map(parse_hex).transpose().context("invalid --service-id")?;
            let request_count = u64::from_str(arguments.value_of("request_count").unwrap_or_default()).context("invalid --request-count")?;
            let max_parallel  = u64::from_str(arguments.value_of("max_parallel").unwrap_or("1")).context("invalid --max-parallel")?;
            let backup_id    = subcommand_arguments.value_of("backup_id").map(parse_hex_bytes::<[u8; 32]>).transpose().context("invalid --backup-id")?;
            ...
            let backup_id = match backup_id {
                Some(backup_id) => {
                    future::Either::A(Ok(backup_id).into_future())
                }
                None => {
                    let token_response = client.get_token(&credentials, &enclave_name)
                                               .map_err(|error| error.context("error during token request"));
                    let backup_id      = token_response.map(|token_response: GetTokenResponse| {
                        token_response.backupId.into()
                    });
            ...
            let request_loop = move |()| {
                ...
                let response = client.backup_request(&credentials, &enclave_name, request)
        ...
        "backup" | "restore" => {
            ...
            let service_id    = leak_argument(&subcommand_arguments.value_of("service_id").map(parse_hex).transpose().context("invalid --service-id")?);
            let request_count = u64::from_str(arguments.value_of("request_count").unwrap_or_default()).context("invalid --request-count")?;
            let max_parallel  = u64::from_str(arguments.value_of("max_parallel").unwrap_or("1")).context("invalid --max-parallel")?;
            let backup_data   = parse_argument(subcommand_arguments.value_of("backup_data"), parse_hex_bytes::<[u8; 32]>).context("invalid --backup-data")?;
            let backup_pin    = parse_argument(subcommand_arguments.value_of("backup_pin"), parse_hex_bytes::<[u8; 32]>).context("invalid --backup-pin")?;
            let backup_tries  = parse_argument(subcommand_arguments.value_of("backup_tries"), u32::from_str).context("invalid --backup-tries")?;
            let valid_from    = SystemTime::now().duration_since(SystemTime::UNIX_EPOCH)
            ...
            let request_loop = move |state: RequestLoopState| {
                let RequestLoopState { request_state, mut responses } = state;
                ...
                let request_state = if let Some(request_state) = request_state {
                    ...
                    let credentials = calculate_credentials(username.clone().unwrap_or_else(rand_username), password.clone(), &token_secret);

                    debug!("fetching token for {}", &credentials.username);

                    let token_response = client.get_token(&credentials, enclave_name)
                                               .map_err(|error| error.context("error during token request").into());
                    let request_state  = token_response.map(move |token_response: GetTokenResponse| {
                        RequestState {
                            credentials,
                ...
                let looped = request_state.and_then(move |request_state: RequestState| {
                    let RequestState { credentials, backup_id, token } = request_state;
                    ...
                    let looped   = response.and_then(move |response: kbupd_client::Response| {
                        match subcommand_name {
                            "backup" => {
                                ...
                                let state = RequestLoopState {
                                    request_state: Some(RequestState { credentials, backup_id, token }),
                                    responses
                                };
...
fn parse_hex_bytes<T>(hex: &str) -> Result<T, failure::Error> where T: AsRef<[u8]> + AsMut<[u8]> + Default {
    let mut buf = Cursor::new(T::default());
    if hex.len() == buf.remaining_mut().saturating_mul(2) {
        parse_hex_buf(hex, &mut buf)?;
        Ok(buf.into_inner())
    } else {
        Err(format_err!("wrong size hexadecimal parameter {} != {}", hex.len(), buf.remaining_mut().saturating_mul(2)))
    }
}
...
fn parse_hex_buf(mut hex: &str, bytes: &mut impl BufMut) -> Result<(), failure::Error> {
    while let Some(hex_byte_str) = hex.get(..2) {
        let hex_byte = u8::from_str_radix(hex_byte_str, 16)
            .with_context(|_| format_err!("invalid hexadecimal byte '{}': {}", hex_byte_str, hex))?;
        bytes.put(hex_byte);
...
fn rand_bytes<T>() -> T where T: AsMut<[u8]> + Default {
...
fn parse_arguments() -> clap::ArgMatches<'static> {
    ...
    let connect_argument =
        clap::Arg::with_name("connect_uri")
        .takes_value(true)
        .required(true)
        .long("connect")
    ...
    let insecure_argument =
        clap::Arg::with_name("insecure")
        .long("insecure")
        .help("Allow insecure server connections when using SSL");

    let username_argument =
        clap::Arg::with_name("username")
        .takes_value(true)
        .required_unless("token_secret")
        .long("username")
        .value_name("username")
        .help("Username to authenticate with, as assigned by a Signal server. Randomly generated by default, if --token-secret is provided.");
    ...
    let token_secret_argument =
        clap::Arg::with_name("token_secret")
        .takes_value(true)
        .long("token-secret")
        .value_name("hex_secret")
        .help("Secret used to generate auth tokens, as a hexadecimal byte string");

    let authentication_secret_group =
        clap::ArgGroup::with_name("auth_secret")
        .required(true)
        .arg("password")
        .arg("token_secret");
