...
use cookie::{Cookie};
use failure::{Fail, format_err, ResultExt};
use futures::prelude::*;
use http::{Uri};
use http::header;
use http::header::{HeaderValue};
use http::response;
use hyper::{Body, Chunk, Method, Response, Request};
...
impl KeyBackupApiClient {
    pub fn new(base_uri: Uri, insecure_ssl: bool) -> Result<Self, failure::Error> {
        let tls_connector = TlsConnector::builder().min_protocol_version(Some(Protocol::Tlsv12))
        ...
        -> impl Future<Item = GetTokenResponse, Error = failure::Error> + Send + 'static
    {
        ...
        let uri_path_and_query   = try_future!(format!("/v1/token/{}", enclave_name).parse::<http::uri::PathAndQuery>().context("error creating request path"));
        ...
        -> impl Future<Item = kbupd_client::Response, Error = failure::Error> + Send + 'static
    {
        ...
        let attestation_response = self.get_attestation(credentials, enclave_name, attestation_request)
        ...
        let response = attestation_response.and_then(move |(attestation_response_cookies, attestation_response): (Vec<Cookie>, RemoteAttestationResponse)| {
            ...
            let (encrypted_request, pending_request) =
                try_future!(client.request(&mut rand::thread_rng(), negotiation, request));
            ...
            let key_backup_response = state.put_backup_request(&credentials, &enclave_name, attestation_response_cookies, key_backup_request)
    ...
    {
        let mut uri_parts        = self.base_uri.clone().into_parts();
        let uri_path_and_query   = try_future!(format!("/v1/attestation/{}", enclave_name).parse::<http::uri::PathAndQuery>().context("error creating request path"));
        uri_parts.path_and_query = Some(uri_path_and_query);
        let uri                  = try_future!(Uri::from_parts(uri_parts).context("error creating request uri"));
        let cookies              = Vec::new();

        let response_with_parts   = self.put_request(uri, credentials, cookies, request);
        let response_with_cookies = response_with_parts.and_then(|(response_parts, response): (response::Parts, RemoteAttestationResponse)| {
            let cookie_headers = response_parts.headers.get_all(header::SET_COOKIE);
            let cookies        = cookie_headers.into_iter().map(|cookie_header: &HeaderValue| -> Result<Cookie<'static>, failure::Error> {
    ...
    {
        let mut uri_parts        = self.base_uri.clone().into_parts();
        let uri_path_and_query   = try_future!(format!("/v1/backup/{}", enclave_name).parse::<http::uri::PathAndQuery>().context("error creating request path"));
        uri_parts.path_and_query = Some(uri_path_and_query);
        let uri                  = try_future!(Uri::from_parts(uri_parts).context("error creating request uri"));
        let response_with_parts  = self.put_request(uri, credentials, cookies, request);
        let response             = response_with_parts.map(|(_parts, response)| response);
    ...
    fn put_request<RequestTy, ResponseTy>(
        ...
        -> impl Future<Item = (response::Parts, ResponseTy), Error = failure::Error> + Send + 'static
    where RequestTy:  Serialize + 'static,
          ResponseTy: for<'de> Deserialize<'de> + Send + 'static,
    {
        ...
        *hyper_request.uri_mut()    = uri;
        hyper_request.headers_mut().insert(header::CONTENT_TYPE,  HeaderValue::from_static("application/json"));
        hyper_request.headers_mut().insert(header::AUTHORIZATION, credentials.into());

        for cookie in cookies {
            let cookie_header_value = try_future!(HeaderValue::from_str(&format!("{}={}", cookie.name(), cookie.value())).context("invalid cookie from server attestation response"));
    ...
    where ResponseTy: for<'de> Deserialize<'de> + Send + 'static,
    {
        ...
        let response_data    = response_body.concat2()
                                            .map_err(failure::Error::from);
        let decoded_response = response_data.and_then(|full_response: Chunk| {
            match serde_json::from_slice(&full_response) {
                Ok(decoded_response) => Ok((response_parts, decoded_response)),
                Err(error) => {
                    debug!("invalid server response: {}\n{}", &error, String::from_utf8_lossy(&full_response.to_vec()));
...
impl From<&KeyBackupApiCredentials> for HeaderValue {
    fn from(from: &KeyBackupApiCredentials) -> Self {
        ...
        HeaderValue::from_str(&authorization_header)
            .unwrap_or_else(|error| panic!("invalid authorization header: {}", error))
    }
}
