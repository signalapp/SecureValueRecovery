...
pub fn proxy_pipe_pair() -> Result<(ProxyPipeStream, TestPipeStream), failure::Error> {
    ...
    Ok((ProxyPipeStream::new(source_pipe_0_rx, source_pipe_1_tx).expect(error_line!()),
...
impl TestCa {
    ...
    pub fn to_pkcs12(&self) -> Result<Pkcs12, failure::Error> {
        ...
        pkcs12.ca(ca);
        let pkcs12 = pkcs12.build("", "kbuptlsd_test_pkcs12", &self.private_key, &self.certificate).context(error_line!())?;
        Ok(pkcs12)
    }
    ...
    pub fn generate_signed_certificate(&mut self, subject_name_str: &str, is_server: bool) -> Result<TestCaSignedCertificate, failure::Error> {
        let private_rsa_key = Rsa::generate(2048).context(error_line!())?;
        let private_key     = PKey::from_rsa(private_rsa_key).context(error_line!())?;
        let certificate     = self.sign_certificate(&private_key, subject_name_str, is_server).context(error_line!())?;
        Ok(TestCaSignedCertificate {
            private_key,
            certificate,
        })
    }

    pub fn sign_certificate(&mut self, pubkey: &PKeyRef<impl HasPublic>, subject_name_str: &str, is_server: bool) -> Result<X509, failure::Error> {
        let valid_from_time  = Asn1Time::days_from_now(0).context(error_line!())?;
        let valid_until_time = Asn1Time::days_from_now(1).context(error_line!())?;
        ...
        let mut certificate = X509Builder::new().context(error_line!())?;
        let () = certificate.set_issuer_name(self.certificate.subject_name()).context(error_line!())?;
        let () = certificate.set_not_before(&valid_from_time).context(error_line!())?;
        let () = certificate.set_not_after(&valid_until_time).context(error_line!())?;
        ...
        if is_server && !subject_name_str.is_empty() {
            ...
            let subject_alt_name = SubjectAlternativeName::new().dns(subject_name_str)
        ...
        let () = certificate.sign(&self.private_key, MessageDigest::sha256()).context(error_line!())?;
