...
fn test_valid() {
    ...
    let mut client_tls_stream = client_tls_connector.connect("kbuptlsd_test_server", client_tcp_stream).expect(error_line!());
...
fn test_no_client_certificate() {
    let mut test_child       = TestChild::new("kbuptlsd_test_ca").expect(error_line!());
    let client_tcp_stream    = test_child.start_child("kbuptlsd_test_server").expect(error_line!());
    let client_tls_connector = TlsConnector::builder().min_protocol_version(Some(Protocol::Tlsv12))
                                                      .danger_accept_invalid_hostnames(true)
                                                      .add_root_certificate(test_child.ca.to_native_tls_certificate().expect(error_line!()))
                                                      .build()
                                                      .expect(error_line!());
    client_tls_connector.connect("kbuptlsd_test_server", client_tcp_stream).expect_err(error_line!());
}
...
fn test_self_signed_client_certificate() {
    ...
    let client_tls_connector = TlsConnector::builder().identity(client_identity)
...
fn build_client(test_ca: &mut TestCa, valid_chain: bool) -> Result<TlsConnector, failure::Error> {
    let client_cert    = test_ca.generate_signed_certificate("kbupdtlsd_test_client", false).expect(error_line!());
    ...
    let client_connector = TlsConnector::builder().identity(client_identity)
                                                  .min_protocol_version(Some(Protocol::Tlsv12))
                                                  .danger_accept_invalid_hostnames(true)
...
impl TestChild {
    ...
    fn start_child(&mut self, subject_name: &str) -> Result<TcpStream, failure::Error> {
        ...
        let (target_stream_0,
             target_stream_1) = util::proxy_pipe_pair().expect(error_line!());
        thread::spawn(move || {
