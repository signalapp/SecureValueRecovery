...
use super::stream::{ProxyRead, ProxyWrite, ProxyStreamError};
...
impl TlsAcceptor {
    pub fn new(tls_identity: Identity, tls_ca_cert: CaCertificate) -> Result<Self, failure::Error> {
        ...
        acceptor.set_private_key(&tls_identity.pkcs12.pkey).context("error setting server private key")?;
        acceptor.set_certificate(&tls_identity.pkcs12.cert).context("error setting server certificate")?;
        if let Some(chain) = tls_identity.pkcs12.chain {
            for cert in chain.iter().rev() {
                acceptor.add_extra_chain_cert(cert.to_owned()).context("error adding server certificate chain")?;
            }
        }

        acceptor.set_min_proto_version(Some(ssl::SslVersion::TLS1_2)).context("error setting minimum tls version")?;
        acceptor.set_max_proto_version(Some(ssl::SslVersion::TLS1_2)).context("error setting maximum tls version")?;
        ...
        match tls_ca_cert {
            CaCertificate::System => {
                return Err(failure::format_err!("cannot use system ca certificates for client certificate validation"));
            }
            CaCertificate::Custom { x509: tls_ca_cert_x509 } => {
                cert_store.add_cert(tls_ca_cert_x509).context("error adding custom ca certificate")?;
            }
        }
        acceptor.set_verify_cert_store(cert_store.build()).context("error setting ca certificate store")?;
...
impl TlsConnector {
    pub fn new(maybe_tls_identity: Option<Identity>,
               tls_hostname:       TlsHostname,
               tls_ca_certs:       Vec<CaCertificate>)
               -> Result<Self, failure::Error>
    {
        let mut connector = ssl::SslConnector::builder_no_default_verify_paths(ssl::SslMethod::tls()).context("error creating connector")?;
        if let Some(tls_identity) = maybe_tls_identity {
            connector.set_private_key(&tls_identity.pkcs12.pkey).context("error setting client private key")?;
            connector.set_certificate(&tls_identity.pkcs12.cert).context("error setting client certificate")?;
            if let Some(chain) = tls_identity.pkcs12.chain {
                for cert in chain.iter().rev() {
                    connector.add_extra_chain_cert(cert.to_owned()).context("error adding client certificate chain")?;
                }
            }
        }

        connector.set_min_proto_version(Some(ssl::SslVersion::TLS1_2)).context("error setting minimum tls version")?;
        connector.set_max_proto_version(Some(ssl::SslVersion::TLS1_2)).context("error setting maximum tls version")?;
        ...
        connector.set_verify_cert_store(cert_store.build()).context("error setting ca certificate store")?;
    ...
    pub fn connect<T: Read + Write>(&self, stream: T) -> Result<TlsStream<T>, HandshakeError<T>> {
        let mut connect_config = self.connector.configure().map_err(|error| HandshakeError::Failure(HandshakeFailure::SetupError(error)))?;
        ...
        connect_config.connect(hostname, stream)
                      .map(TlsStream::new)
                      .map_err(HandshakeError::from)
    }
}
...
fn openssl_to_proxy_stream_result<T>(result: Result<T, ssl::Error>) -> Result<T, ProxyStreamError> {
    match result {
        ...
        Err(error) => {
            let error = error.into_io_error().unwrap_or_else(|error| io::Error::new(io::ErrorKind::Other, error));
...
impl<T> From<ssl::HandshakeError<T>> for HandshakeError<T> {
    fn from(error: ssl::HandshakeError<T>) -> Self {
        match error {
            ...
            ssl::HandshakeError::WouldBlock(stream)  => {
